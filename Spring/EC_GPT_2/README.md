...
## response 1:
To fix the issue where normal users can access the `/api/hiddenmessage` endpoint, you need to update your security configuration to restrict access to this endpoint only to users with the `ROLE_ADMIN` authority. Currently, you're granting access to this endpoint to users with `ROLE_USER` authority, which includes normal users.
...
With this change, only users with the `ROLE_ADMIN` authority will be able to access the `/api/hiddenmessage` endpoint, while users with `ROLE_USER` authority will only be able to access the `/api/person` endpoint.